...
//                 }
				$suffix1 = implode('.',array_slice(explode('.', $_SERVER['HTTP_HOST']),-2));
				$suffix2 = array_slice(explode('.', $_SERVER['HTTP_HOST']),-1)[0];
				if(in_array($suffix1, C('APP_DOMAIN_SUFFIX_ARRAY'))){
					C('APP_DOMAIN_SUFFIX',$suffix1);
				}else if(in_array($suffix2, C('APP_DOMAIN_SUFFIX_ARRAY'))){
					C('APP_DOMAIN_SUFFIX',$suffix2);
				}
                $countDian = substr_count(C('APP_DOMAIN_SUFFIX'),'.');
                if($countDian > 0){ //支持无限级子域名部署
                ...
                if(!empty($domain)) {
                    ...
                    $domain2   = strtolower(array_pop($domain)); // 二级域名
                    if($domain) { // 存在三级域名
